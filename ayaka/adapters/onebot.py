...
async def send_many(id: str, msgs: list[str]):
    ...
    for i in range(div_cnt):
        ...
        await bot.send_group_forward_msg(group_id=id, messages=msgs)
...
async def get_member_list(gid: str):
    bot = get_current_bot()
    try:
        users = await bot.get_group_member_list(group_id=gid)
        return [
            User(id=user["user_id"], name=user["card"]
                 or user["nickname"], role="admin")
            for user in users
        ]
    except:
        pass
